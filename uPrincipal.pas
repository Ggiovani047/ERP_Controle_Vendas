...
type
  TfrmPrincipal = class(TForm)
    ...
    btnAnaliseDeDesempenho: TSpeedButton;
    ...
    procedure btnAnaliseDeDesempenhoClick(Sender: TObject);
...
procedure TfrmPrincipal.btnAnaliseDeDesempenhoClick(Sender: TObject);
begin
...
begin
  if MessageDlg('Tem Certeza que deseja fechar o sistema? ',mtInformation, [mbYes, mbNo], 0) = mrYes then
    Application.Terminate;
end;
...
begin
   if MessageDlg('Tem Certeza que deseja fazer Logout? ',mtInformation, [mbYes, mbNo], 0) = mrYes then begin
     try
      if Assigned(oUsuarioLogado) then
        FreeAndNil(oUsuarioLogado);
